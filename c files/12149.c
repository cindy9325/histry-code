...
List* zip(List *lptr1, List *lptr2)
{
    ...
    if ( lptr1 == NULL || lptr2 == NULL ) { //lptr1 or lptr2 is NULL
        return NULL;
    } else {
        a.pair = (Pair*) malloc(sizeof(Pair)); // use malloc to allocate a space for Pair
        a.pair->left = copy_atom(head(lptr1)); // use copy_atom to copy lptr1's first atom
        a.pair->right = copy_atom(head(lptr2)); // use copy_atom to copy lptr2's first atom
        a.dtype = 2;
        l1 = zip(lptr1->next, lptr2->next); // use zip to deal with the rest of the lists  (Hint: use recursive)
        l2 = cons(&a, l1); // use cons to create a new list
        free_pair(a.pair);
        free_list(l1);
        return l2;
    }
...
Pair* unzip(List *lptr)
{
    ...
    h = head(lptr); // // take the first atom of lptr
    p = unzip(lptr->next); // use unzip to deal with the rest of lptr recursively
    l1 = cons(h->pair->left, p->left->lst); // the two lists are from two separate cons
    l2 = cons(h->pair->right, p->right->lst); // one cons generates the list on the left, the other generates the list on the right
    newp = pair_list(l1, l2); // // use pair_list to generate a new pair from the two lists
    free_list(l1);
    free_list(l2);
    free_pair(p);
    return newp;
